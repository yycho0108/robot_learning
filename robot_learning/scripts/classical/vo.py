...
from vo_common import recover_pose, recover_pose_from_RT
from vo_common import oriented_cov
from vo_common import print_Rt

from vo_math import robust_mean 
from landmarks import Landmarks
from conversions import Conversions
from vgraph import VGraph

from opt.ba import ba_J, ba_J_v2, schur_trick
from opt.pnp import solve_PNP, PNPSolverRANSAC
from opt.tri import solve_TRI, solve_TRI_fast

from viz import drawlines, print_ratio, drawMatches, get_points_color
...
class ClassicalVO(object):
    ...
    def run_PNP(self, pt3_map, pt2_cam, pose,
        ...
        if True: # == USE_CUSTOM
            pnp_it, pose_pnp, _ = self.pnp_(pt3_map, pt2_cam,
            ...
            #        )
            if pose_pnp is None:
                #pfig = self.fig_['pnp-dbg']
                #pfig2 = self.fig_['pnp-dbg2']

                #pax  = pfig.gca()
                #pax.cla()
                #pax.plot(pt2_cam[:,0], pt2_cam[:,1], '+')

                #pax2 = pfig2.gca(projection='3d')
                #pax2.cla()
                #pax2.plot(pt3_map[:,0], pt3_map[:,1], pt3_map[:,2], '+')
                #plt.pause(0.001)
