...
def load_chair1(data_root, index, size=None):
    ...
    if size is not None:
        img2  = cv2.resize(img2, (cfg.IMG_WIDTH, cfg.IMG_HEIGHT)) # 320x240 u8

    return (img1, img2, flow)

def load_chair(data_root, n):
    idx = 1 + np.random.choice(22872, size=n, replace=False)
    data = zip(*[load_chair1(data_root, i) for i in idx])
...
def load_ilsvrc(data_root, n):
    idx = 1 + np.random.choice(3000, size=n, replace=False)
    data = zip(*[load_ilsvrc1(data_root, i) for i in idx])
    img1, img2, flow = [np.stack(e, axis=0) for e in data]
    return (img1, img2, flow)
